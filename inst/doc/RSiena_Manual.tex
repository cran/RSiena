...
Christian Steglich, Johan Koskinen, and Mark Ortmann.
...
\item Use \sfn{sienaAlgorithmCreate()} to create a \sfn{sienaAlgorithm} object, containing
  the specifications for the estimation algorithm.
...
\item Create \SI objects for each network, behavior variable and covariate,
  using the functions \sfn{sienaDependent()} (for both networks and behavior
...
\item Use \sfn{sienaAlgorithmCreate()} to create a a \sfn{sienaAlgorithm} object, containing
  the specifications for the estimation algorithm.
\item Use \sfn{siena07()} to run the estimation procedure.
...
selection considerations, refer to \citet*{Lospinoso2010b} and \citet{Lospinoso2012}.
...
These variables also are defined using the \sfn{sienaDependent} function,
...
individual covariates: here the subtracted values are
...
The Stochastic Actor-Oriented Model for two-mode (or bipartite) networks
...
Alternatively this list can be displayed in a browser by using the function:
...
restricted to triples with the same or different values on a covariate.
\begin{enumerate}
\setcounter{enumi}{\value{savenumi}}
\item
\begin{minipage}[t]{.7\textwidth}
...
\end{verbatim}
Between parentheses: the functionality of these effects is duplicated by
...
\item For the analysis of diffusion of innovations,
  ...
  were constant; see \citet{Greenan12}.
 ...
 and the effect is $a_i(x)$.
  ...
  \begin{enumerate}
  \item the \emph{average exposure effect}
 ...
 \[ \exp\Big( \alpha_h \, \big( \sum_j z_j \, x_{ij}\big)/\big(\sum_j x_{ij} \big) \Big)\, . \]
  ...
  in the rate function  leads to a
...
\noindent
\emph{Solutions.}\\
1. Check whether your model specification is reasonable;
...
Changes in RSiena:
  \begin{itemize}
  \item The possibility to use the obsolete packages \sfn{snow} and \sfn{rlecuyer}
     for R versions older than 2.14.0 was dropped; their functionality was replaced by
     package \sfn{parallel}.
...
For RSiena and RSienaTest:
  \begin{itemize}
	...
	\item GWESP effects: default parameter changed from 25 to 69
		(corresponding to $\alpha = \log(2)$.) See earlier in this the manual.
